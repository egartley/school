...
public final class DrawingPanel implements ImageObserver {
	...
	public static final int PIXEL_ALPHA = 0xff000000; // rgb integer for alpha
														// 100% opacity
	public static final int PIXEL_BLUE = 0x000000ff; // rgb integer for 100%
														// blue
	public static final int PIXEL_GREEN = 0x0000ff00; // rgb integer for 100%
														// green
	...
	static {
		try {
			String debugProp = String.valueOf(
					System.getProperty("drawingpanel.debug")).toLowerCase();
	...
	/*
	 * Called when DrawingPanel class loads up. Checks whether the user wants to
	...
	/**
	 * Returns the red component of the given RGB pixel from 0-255. Often used
	 * in conjunction with the methods getPixelRGB, setPixelRGB, etc.
	 */
	...
	/**
	 * Returns the green component of the given RGB pixel from 0-255. Often used
	 * in conjunction with the methods getPixelRGB, setPixelRGB, etc.
	 */
	...
	/**
	 * Returns the blue component of the given RGB pixel from 0-255. Often used
	 * in conjunction with the methods getPixelRGB, setPixelRGB, etc.
	 */
	...
	private static boolean hasProperty(String name) {
		try {
			...
			if (DEBUG)
				System.out.println("Security exception when trying to read "
						+ name);
			return false;
		}
	}
	...
	/*
	 * Returns whether the 'main' thread is still running. Used to determine
	 * whether to exit the program when the drawing panel is closed by the user.
	 */
	public static boolean mainIsActive() {
		...
		for (int i = 0; i < threads.length; i++) {
			...
			if (DEBUG)
				System.out.println("    DrawingPanel.mainIsActive(): "
						+ thread.getName() + ", priority="
						+ thread.getPriority() + ", alive=" + thread.isAlive()
						+ ", stack="
	...
	/*
	 * Returns whether the given Java system property has been set to a "truthy"
	 * value such as "yes" or "true" or "1".
	 */
		...
		try {
			...
			return prop != null
					&& (prop.equalsIgnoreCase("true")
							|| prop.equalsIgnoreCase("yes") || prop
								.equalsIgnoreCase("1"));
		} catch (SecurityException e) {
			if (DEBUG)
				System.out.println("Security exception when trying to read "
	...
	/**
	 * Returns an RGB integer made from the given red, green, and blue
	 * components from 0-255. The returned integer is suitable for use with
	 * various RGB integer methods in this class such as setPixel.
	 */
	public static int toRgbInteger(int r, int g, int b) {
		return 0xff000000 | ((r & 0x000000ff) << 16) | ((g & 0x000000ff) << 8)
				| ((b & 0x000000ff));
	}

	/*
	 * Returns whether the current program is running in the DrJava editor. This
	 * was needed in the past because DrJava messed with some settings.
	 */
	...
	private Map<String, Integer> counts; // new field to support
											// DebuggingGraphics
	...
	/**
	 * Constructs a drawing panel of given width and height enclosed in a
	 * window.
	 * 
	 * @param width
	...
	public DrawingPanel(int width, int height) {
		...
		if (DEBUG)
			System.out.println("DrawingPanel(): going to grab lock");
		synchronized (LOCK) {
			instances++;
			...
			if (shutdownThread == null && !usingDrJava()) {
				if (DEBUG)
					System.out.println("DrawingPanel(): starting idle thread");
					...
					public void run() {
						...
						try {
							while (true) {
								...
								if (DEBUG)
									System.out
											.println("DrawingPanel idle thread: instances="
													+ instances
													+ ", save="
													+ save
													+ ", main active="
								...
								if ((instances == 0 || save) && !mainIsActive()) {
									try {
										...
										if (DEBUG)
											System.out
													.println("DrawingPanel idle thread: unable to exit program: "
															+ sex);
									}
								}
							...
							if (DEBUG)
								System.out
										.println("DrawingPanel idle thread: exception caught: "
												+ e);
						}
					}
				});
		...
		if (DEBUG)
			System.out.println("DrawingPanel(w=" + width + ",h=" + height
					+ ",anim=" + isAnimated() + ",graph=" + isGraphical()
					+ ",save=" + shouldSave());
			...
			try {
				Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
					...
					public void run() {
						if (DEBUG)
							System.out
									.println("DrawingPanel.run(): Running shutdown hook");
						if (DEBUG)
							System.out
									.println("DrawingPanel shutdown hook: instances="
											+ instances);
						try {
							...
							System.err
									.println("Security error while saving image: "
											+ e);
						} catch (IOException e) {
				...
				if (DEBUG)
					System.out
							.println("DrawingPanel(): unable to add shutdown hook: "
									+ e);
			}
		}
	}
	...
	/*
	 * Whether the panel should automatically switch to animated mode if it
			...
			if (filenames.isEmpty()) {
				...
				if (filenames.size() == 1) {
					...
					int choice = showOptionDialog(frame,
							"File to compare against?", "Choose File",
	 ...
	 * occurrences calls on various drawing methods. Experts can manipulate the
	 * map, but novices will be encouraged to simply print it.
	 */
	...
	/**
	 * A variation of getGraphics that returns an object that records a count
	 * for various drawing methods. See also: getCounts.
	 */
	...
	private BufferedImage getImage() {
		...
		// if (DEBUG)
		// System.out.println("DrawingPanel getImage setting background to " +
		// backgroundColor);
		g.setColor(backgroundColor);
	...
	/**
	 * Returns the color of the pixel at the given x/y coordinate as a Color
	 * object. If nothing has been explicitly drawn on this particular pixel,
	 * the panel's background color is returned.
	 */
	...
	/**
	 * Returns the color of the pixel at the given x/y coordinate as an RGB
	 * integer. The individual red, green, and blue components of the RGB
	 * integer can be extracted from this by calling DrawingPanel.getRed,
	 * getGreen, and getBlue. If nothing has been explicitly drawn on this
	 * particular pixel, the panel's background color is returned. See also:
	 * getPixel.
	 */
	...
	/**
	 * Returns the colors of all pixels in this DrawingPanel as a 2-D array of
	 * Color objects. The first index of the array is the y-coordinate, and the
	 * second index is the x-coordinate. So, for example, index [r][c]
	 * represents the RGB pixel data for the pixel at position (x=c, y=r).
	 */
	...
	/**
	 * Returns the colors of all pixels in this DrawingPanel as a 2-D array of
	 * RGB integers. The first index of the array is the y-coordinate, and the
	 * second index is the x-coordinate. So, for example, index [r][c]
	 * represents the RGB pixel data for the pixel at position (x=c, y=r). The
	 * individual red, green, and blue components of each RGB integer can be
	 * extracted from this by calling DrawingPanel.getRed, getGreen, and
	 * getBlue.
	 */
	...
	/**
	 * Returns the drawing panel's current zoom factor. Initially this is 1 to
	 * indicate 100% zoom, the original size.
	 */
	public int getZoom() {
		return currentZoom;
	}
	...
	/**
	 * Required method of ImageObserver interface. Notifies the panel when
	 * images are loaded and updated.
	 */
	@Override
	...
	private void initializeAnimation() {
		 ...
		 * encoder.startEncoding(stream); } catch (IOException e) {
	 ...
	 * This is almost always true except in some server environments where the
	 * DrawingPanel is run 'headless' without a GUI, often for scripting and
	 * automation purposes.
	 */
	...
	/*
	 * Returns true if the drawing panel class is in multiple mode. This would
	 * be true if the current program pops up several drawing panels and we want
	 * to save the state of each of them to a different file.
	 */
	...
	/**
	 * Loads an image from the given file on disk and returns it as an Image
	 * object. Throws a RuntimeException if the given file is not found.
	 */
	...
	/*
	 * Returns whether the drawing panel should be closed and the program should
	 * be shut down.
	 */
	private boolean readyToClose() {
		/*
		 * if (isAnimated()) { // wait a little longer, in case animation is
		 * sleeping return System.currentTimeMillis() > createTime + 5 * DELAY;
		 * } else { return System.currentTimeMillis() > createTime + 4 * DELAY;
		 * }
		 */
	 ...
	 */
	private void replaceColor(BufferedImage image, Color oldColor,
			Color newColor) {
	...
	/**
	 * Takes the current contents of the drawing panel and writes them to the
	 * given file.
	 */
	...
	public void saveAnimated(String filename) throws IOException {
		...
		if (DEBUG)
			System.out.println("DrawingPanel.saveAnimated(" + filename + ")");
	...
	/*
	 * Called when the user presses the "Save As" menu item. Pops up a file
	 * chooser prompting the user to save their panel to an image.
	 */
	private void saveAs() {
	...
	/*
	 * Called when the user presses the "Save As" menu item on an animated
	 * panel. Pops up a file chooser prompting the user to save their panel to
	 * an image.
	 */
	...
	/*
	 * A helper method to facilitate the Save As action for both animated and
	 * non-animated images.
	 */
	...
	/*
	 * Saves the drawing panel's image to a temporary file and returns that
	 * file's name.
	 */
	...
	/**
	 * Sets the drawing panel's height in pixels to the given value. After
	 * calling this method, the client must call getGraphics() again to get the
	 * new graphics context of the newly enlarged image buffer.
	 */
	...
	/**
	 * Sets the color of the pixel at the given x/y coordinate to be the given
	 * color. If the color is null, the call has no effect.
	 */
	 ...
	 * represented by the given RGB integer. See also: setPixel.
	 */
	...
	/**
	 * Sets the colors of all pixels in this DrawingPanel to the colors in the
	 * given 2-D array of Color objects. The first index of the array is the
	 * y-coordinate, and the second index is the x-coordinate. So, for example,
	 * index [r][c] represents the RGB pixel data for the pixel at position
	 * (x=c, y=r). If the given array's dimensions do not match the width/height
	 * of the drawing panel, the panel is resized to match the array. If the
	 * pixel array is null or size 0, the call has no effect. If any rows or
	 * colors in the array are null, those pixels will be ignored. The 2-D array
	 * passed is assumed to be rectangular in length (not jagged).
	 */
	 ...
	 * represented by the given 2-D array of RGB integers. The first index of
	 * the array is the y-coordinate, and the second index is the x-coordinate.
	 * So, for example, index [r][c] represents the RGB pixel data for the pixel
	 * at position (x=c, y=r). If the given array's dimensions do not match the
	 * width/height of the drawing panel, the panel is resized to match the
	 * array. If the pixel array is null or size 0, the call has no effect. The
	 * 2-D array passed is assumed to be rectangular in length (not jagged).
	 */
	 ...
	 * After calling this method, the client must call getGraphics() again to
	 * get the new graphics context of the newly enlarged image buffer.
	 */
	...
	/**
	 * Sets the drawing panel's width in pixels to the given value. After
	 * calling this method, the client must call getGraphics() again to get the
	 * new graphics context of the newly enlarged image buffer.
	 */
	...
	/*
	 * Returns whether the user wants to save the drawing panel contents to a
	 * file automatically.
	 */
	...
	/*
	 * Shows a dialog box with the given choices; returns the index chosen (-1
	 * == canceled).
	 */
	private int showOptionDialog(Frame parent, String title, String message,
			final String[] names) {
	...
	/**
	 * Zooms the drawing panel in/out to the given factor. A zoom factor of 1,
	 * the default, indicates normal size. The factor value passed should be >=
	 * 1; if not, 1 will be used.
	 */
	public void zoom(int zoomFactor) {
		...
		if (isGraphical()) {
			...
			// resize frame if any more space for it exists or it's the wrong
			// size
	...
	/*
	 * Internal action listener for handling events on buttons and GUI
	 * components.
	 */
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof Timer) {
				...
				JOptionPane.showMessageDialog(frame, ABOUT_MESSAGE,
						ABOUT_MESSAGE_TITLE, JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	private class DPKeyEventHandlerAdapter implements KeyListener {
		...
		public DPKeyEventHandlerAdapter(DPKeyEventHandler handler,
				String eventType) {
	...
	private class DPMouseEventHandlerAdapter implements MouseInputListener {
		...
		public DPMouseEventHandlerAdapter(DPMouseEventHandler handler,
				String eventType) {
			this.handler = handler;
			this.eventType = eventType.intern();
		}
	...
	public void onClick(DPMouseEventHandler e) {
		DPMouseEventHandlerAdapter adapter = new DPMouseEventHandlerAdapter(e,
				"click");
	...
	public void onKeyDown(DPKeyEventHandler e) {
		DPKeyEventHandlerAdapter adapter = new DPKeyEventHandlerAdapter(e,
				"press");
	 ...
	 * expected output as a "sanity check" on their program's behavior. Notice
	 * that it extends Graphics and not Graphics2D, so it is more limited than
	 * g2.
	 * 
	 * @author Stuart Reges
	 */
	 ...
	 * screen to report the differences between two images. It is used to allow
	 * the client to compare their program's output against a known correct
	 * output and view which pixels differ between the two.
	 */
	private class DiffImage extends JPanel implements ActionListener,
		...
		public void paintComponent(Graphics g) {
			...
			if (image2 != null) {
				...
				g2.setComposite(AlphaComposite.getInstance(
						AlphaComposite.SRC_ATOP, ((float) opacity) / 100));
			...
			if (highlightDiffs && image1 != null && image2 != null) {
				...
				for (int y = 0; y < hmax; y++) {
					for (int x = 0; x < wmax; x++) {
						int pixel1 = (x < w1 && y < h1) ? image1.getRGB(x, y)
								: 0;
						int pixel2 = (x < w2 && y < h2) ? image2.getRGB(x, y)
								: 0;
						if (pixel1 != pixel2) {
							g2.fillRect(x, y, 1, 1);
						}
		...
		public void setImage1() {
			...
			try {
				...
				countDiffPixels();
				diffPixelsLabel
						.setText("(" + numDiffPixels + " pixels differ)");
		...
		public void setImage1(BufferedImage image) {
			...
			image1 = image;
			setPreferredSize(new Dimension(Math.max(getPreferredSize().width,
					image.getWidth()), Math.max(getPreferredSize().height,
					image.getHeight())));
			if (frame != null) {
				frame.pack();
			}
		...
		private void setupComponents() {
			...
			diffPixelsLabel = new JLabel("(" + numDiffPixels
					+ " pixels differ)");
			diffPixelsLabel.setFont(diffPixelsLabel.getFont().deriveFont(
					Font.BOLD));
		...
		private void setupLayout() {
			...
			southPanel1.setBorder(BorderFactory
			...
			southPanel2.setBorder(BorderFactory
					.createLineBorder(Color.DARK_GRAY));
